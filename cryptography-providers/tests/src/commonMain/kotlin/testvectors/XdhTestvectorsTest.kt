...
abstract class XdhTestvectorsTest(provider: CryptographyProvider) : AlgorithmTest<XDH>(XDH, provider) {
    ...
    private fun rfc7748DiffieHellmanTest(
        ...
        // Verify public keys match their private keys (if supported by provider)
        try {
            val alicePublicFromPrivate = alicePrivate.getPublicKey()
            assertContentEquals(
                alicePublicHex.hexToByteArray(),
                alicePublicFromPrivate.encodeToByteArray(XDH.PublicKey.Format.RAW),
                "Alice's public key should match her private key"
            )
        } catch (e: Throwable) {
            if (!supportsPublicKeyAccess(e)) {
                // Provider doesn't support public key extraction, skip verification
            } else throw e
        }

        try {
            val bobPublicFromPrivate = bobPrivate.getPublicKey()
            assertContentEquals(
                bobPublicHex.hexToByteArray(),
                bobPublicFromPrivate.encodeToByteArray(XDH.PublicKey.Format.RAW),
                "Bob's public key should match his private key"
            )
        } catch (e: Throwable) {
            if (!supportsPublicKeyAccess(e)) {
                // Provider doesn't support public key extraction, skip verification
    ...
    @Test
    fun rfc7748Section52_X25519_Test1() = rfc7748BasicTest(
    ...
    @Test
    fun rfc7748Section52_X25519_Test2() = rfc7748BasicTest(
        curve = XDH.Curve.X25519,
    ...
    @Test
    fun rfc7748Section52_X448_Test1() = rfc7748BasicTest(
    ...
    @Test
    fun rfc7748Section52_X448_Test2() = rfc7748BasicTest(
        curve = XDH.Curve.X448,
