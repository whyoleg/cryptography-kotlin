...
abstract class AesCcmTest(provider: CryptographyProvider) : AesBasedTest<AES.CCM>(AES.CCM, provider) {
    @Test
    fun testSizes() = runTestForEachKeySize {
        ...
        listOf(32, 48, 64, 80, 96, 112, 128).forEach { tagSizeBits ->
            ...
            key.cipher(tagSizeBits.bits).run {
                listOf(0, 15, 16, 17, 319, 320, 321).forEach { inputSize ->
                    ...
                    listOf(7, 8, 12, 13).forEach { ivSize ->
    ...
    fun testCustomIvSize() = runTestForEachKeySize {
        ...
        listOf(7, 8, 10, 12, 13).forEach { ivSizeBytes ->
