...
internal object KeyInfoUnwrap {
    fun unwrapSpkiForOids(der: ByteArray, oids: List<ObjectIdentifier>): ByteArray {
        var lastError: Throwable? = null
        for (oid in oids) {
            try {
                return unwrapSubjectPublicKeyInfo(oid, der)
            } catch (t: Throwable) {
                lastError = t
            }
        }
        throw lastError ?: IllegalArgumentException("No OID matched for SPKI unwrap")
    }
